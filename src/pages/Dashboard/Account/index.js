...
export default function Account() {
  ...
  const getUserData = async (id) => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setNewName(docSnap.data().fullName); // Set initial newName to current fullName
      } else {
  ...
  const updateUserData = async () => {
    try {
      ...
      setIsEditing(false);
      window.toastify("Name updated successfully", "success");
    } catch (error) {
      console.log(error);
      window.toastify("Error updating name", "error");
    }
  };
  ...
  return (
    ...
    <div className="card mt-4">
      ...
      <div className="card__subtitle">{user.role}</div>
      <div className="card__wrapper">
        <button className="card__btn my-3 me-2" onClick={handleLogout}>
          Logout
        </button>
        <Button
            icon={<EditOutlined />}
